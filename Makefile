...
ifeq ($(shell uname), FreeBSD)
   ...
   LIBS=-L$(LOCALBASE)/lib -L. -Llibelf-0.8.13/lib/ -lelf $(PTHREAD_LIBS) $(LDFLAGS)
else
...
#   PCAP_LIB=-lpcap
   ...
   LIBS=-L/usr/lib -L. -ldl -lrt -Llibelf-0.8.13/lib/ -lelf $(PTHREAD_LIBS)
   DESTDIR=/usr
else
ifeq ($(shell uname -s), Darwin)
   OSNAME=Darwin
   CFLAGS+=-I/usr/local/include -I/opt/local/include -mdynamic-no-pic -D_FILE_OFFSET_BITS=64
   LIBS=-L/usr/local/lib -L/opt/local/lib -L. -ldl -Llibelf-0.8.13/lib/ -lelf -lpthread
...
ifeq ($(shell uname -o), Cygwin)
   ...
   zLIBS=-L. -lelf -lpthread -lws2_32 -lwsock32 -lmingwex  drmingw-0.8.0-win32\lib\libexchndl.a -static-libgcc -static
   LIBS=-L. -lpthread -lws2_32 -lwsock32 -lmingwex  drmingw-0.8.0-win32\lib\libexchndl.a -static-libgcc -static
endif
endif
endif
endif
...
SLIRP_OBJ = bootp.o cksum.o debug.o if.o  ip_icmp.o ip_input.o ip_output.o mbuf.o \
	misc.o queue.o sbuf.o slirp.o socket.o tcp_input.o tcp_output.o tcp_subr.o \
	tcp_timer.o tftp.o udp.o


.PHONY: all
# all: $(PROG) nvram_export$(BIN_EXT) udp_send$(BIN_EXT) udp_recv$(BIN_EXT)
all: $(PROG) udp_send$(BIN_EXT) udp_recv$(BIN_EXT) slirp_rdr$(BIN_EXT) ethlist$(BIN_EXT)

$(PROG): mips64_microcode_dump.inc ppc32_microcode_dump.inc \
	$(LEX_C) $(C_OBJS) $(A_OBJS)
	@echo "Linking $@"
	$(CC) -o $@ $(C_OBJS) $(A_OBJS) -lelf -Llibelf-0.8.13/lib/ $(LIBS)

udp_send$(BIN_EXT): udp_send.o net.o crc.o utils.o inet_aton.o mman.o
	@echo "Linking $@"
	$(CC) -Wall -o $@ udp_send.o net.o crc.o utils.o inet_aton.o mman.o localtime_r.o $(LIBS)

udp_recv$(BIN_EXT): udp_recv.o net.o crc.o utils.o inet_aton.o mman.o
	@echo "Linking $@"
	$(CC) -Wall -o $@ udp_recv.o net.o crc.o utils.o inet_aton.o mman.o localtime_r.o $(LIBS)


slirp_rdr$(BIN_EXT): slirp_rdr.o net.o crc.o utils.o inet_aton.o mman.o $(SLIRP_OBJ)
	@echo "Linking $@"
	$(CC) -Wall -o $@ slirp_rdr.o net.o crc.o utils.o inet_aton.o mman.o localtime_r.o $(SLIRP_OBJ) -liphlpapi $(LIBS)

ethlist$(BIN_EXT): ethlist.o
	@echo "Linking $@"
	$(CC) -Wall -o $@ ethlist.o -liphlpapi $(LIBS) WpdPack\Lib\wpcap.lib
...
ppc32_microcode_dump.inc: rom2c$(BIN_EXT) microcode/ppc32_test2
...
clean:
	...
	$(RM) -f $(SLIRP_OBJ)
	$(RM) -f slirp_rdr.o   udp_recv.o    udp_send.o
	$(RM) -f slirp_rdr.exe   udp_recv.exe    udp_send.exe
	$(RM) -f ethlist.o ethlist.exe
...
ethlist.o:
	$(CC) -IWpdPack/Include/pcap -I. -c $*.c

bootp.o:
	$(CC) $(CFLAGS) -c contrib\slirp\$*.c
cksum.o:
	$(CC) $(CFLAGS) -c contrib\slirp\$*.c
debug.o:
	$(CC) $(CFLAGS) -c contrib\slirp\$*.c
